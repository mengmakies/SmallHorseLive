...
void UCloudrunSynchronouslyOnContextQueue(UCloudGPUImageContext *context, void (^block)(void));
void UCloudrunAsynchronouslyOnContextQueue(UCloudGPUImageContext *context, void (^block)(void));
void UCloudreportAvailableMemoryForGPUImage(NSString *tag);

@class UCloudGPUImageMovieWriter;
 ...
 */
@interface UCloudGPUImageOutput : NSObject
{
    UCloudGPUImageFramebuffer *outputFramebuffer;
...
@property(readwrite, nonatomic, retain) UCloudGPUImageMovieWriter *audioEncodingTarget;
@property(readwrite, nonatomic, unsafe_unretained) id<UCloudGPUImageInput> targetToIgnoreForUpdates;
@property(nonatomic, copy) void(^frameProcessingCompletionBlock)(UCloudGPUImageOutput*, CMTime);
@property(nonatomic) BOOL enabled;
@property(readwrite, nonatomic) UCloudGPUTextureOptions outputTextureOptions;

/// @name Managing targets
- (void)setInputFramebufferForTarget:(id<UCloudGPUImageInput>)target atIndex:(NSInteger)inputTextureIndex;
- (UCloudGPUImageFramebuffer *)framebufferForOutput;
 ...
 */
- (void)addTarget:(id<UCloudGPUImageInput>)newTarget;
...
/** Adds a target to receive notifications when new frames are available.
 ...
 @param newTarget Target to be added
 */
- (void)addTarget:(id<UCloudGPUImageInput>)newTarget atTextureLocation:(NSInteger)textureLocation;
 ...
 */
- (void)removeTarget:(id<UCloudGPUImageInput>)targetToRemove;
